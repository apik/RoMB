...
#define dia 3
#if dia==1
     RoMB_loop_by_loop B0_1loop_lbl(lst(k),lst(-pow(k,2),-pow(p+k,2)),lst(pow(p,2)==s),lst(2,1));
     B0_1loop_lbl.integrate_map(lst(s==-2,m1s==0,m2s==0),3	);
#elif dia==2
     ...
     B0_1loop_lblm.integrate_map(lst(s==-6,m1s==2,m2s==3),3);
     cout<<"************** From PJfry ***********"<<endl;
     cout<<""<<endl;
     /*
     FRESULT for parameters: {s==-6,m1s==2,m2s==3} 1,2
     
      FRESULT anl :           = 0.1377684395547933982-(351.7917146849281309)*eps^2-(0.17681173179727871503)*eps
       FRESULT num:           = 0.1377684395547933982-(351.7917146849281309)*eps^2-(0.17681173179727871503)*eps
        eps^0 term: 0.1377684395547933982 +/- 1.538714377022556655E-5
         eps^1 term: -0.17681173179727871503 +/- 8.524037286570291705E-5
          eps^2 term: -351.7917146849281309 +/- 0.005518036326090519649
          */
#endif
    //MB works???
